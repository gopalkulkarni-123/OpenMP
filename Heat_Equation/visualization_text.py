...
def save_frame(data, frame, filename):
    plt.imshow(data[frame], cmap='viridis', interpolation='nearest', vmin=-100, vmax=200)
    plt.colorbar()
...
def animate_heatmap(data, interval=100):
    fig, ax = plt.subplots()
    cax = ax.imshow(data[0], cmap='viridis', interpolation='nearest', vmin=-100, vmax=200)
    fig.colorbar(cax)
    ...
    ani.save('Results/trial_csv.mp4', fps=10)

if __name__ == "__main__":
    file_path = 'Results/trial_csv_compare.txt'
